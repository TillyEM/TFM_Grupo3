...
def training_pipeline(path, model_info_db_name='models-db'):
    ...
    # columnas a retirar en la pipeline de DropColumns
    columns_to_drop = model_config['columns_to_drop']
    # Config. de Pipeline pro Modelo
    indDes = model_config['indDes']
    gen = model_config['gen']  
    exprel = model_config['exprel']
    uni = model_config['uni']
    educ = model_config['educ']
    exp = model_config['exp']
    tam = model_config['tam']
    tipo = model_config['tipo']
    ultNT = model_config['ultNT']
    horas = model_config['horas']
    ...
    X_train, X_test, y_train, y_test = make_dataset(path, ts, target, cols_to_remove,columns_to_drop,
